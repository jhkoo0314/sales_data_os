...
- `입력 -> 검증 -> 정규화 -> KPI -> validation -> payload -> builder` 백엔드 엔진 설계
- Supabase 실제 연결
- Python worker 실제 구현
- Builder payload contract 고정
...
현재 기준으로는 기본 화면이 먼저 깔린 상태이므로,
다음 핵심 작업은 아래 백엔드 공식 흐름을 설계하는 것이다.
