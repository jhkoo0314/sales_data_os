...
import { runBuilderRender } from "@/lib/server/builder";
...
export async function POST(
  ...
  try {
    ...
    const result = await runBuilderRender({
      companyKey,
      executionMode: body.execution_mode ?? null
    });
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to render builder reports.";
