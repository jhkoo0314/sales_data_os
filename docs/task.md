...
- 사용자가 현재 어느 회사를 보고 있는지 바로 이해하게 만든다
- 업로드 이후 다음 행동이 무엇인지 화면이 직접 설명하게 만든다
...
- `Phase 10` worker runtime 완료
- 회사 / 업로드 / intake / pipeline API 응답 구조 고정
- `company_key` 기준 source 저장 구조 유지

세부 구현 계획:

1. `Workspace`를 실제 회사 진입 화면으로 연결
   - 회사 목록을 mock이 아니라 실제 `companies` 또는 회사 registry 기준으로 조회
   - 카드/행마다 최근 run 상태, 최근 업로드 시각, 현재 진행 가능 여부를 함께 노출
   - 단순 상태 배지 대신 "업로드 준비가 됨", "필수 파일이 부족함", "최근 실행이 완료됨" 같은 설명 문장을 같이 붙임

2. 전역 회사 문맥 고정
   - 상단 공통 영역에서 현재 `company_key`를 계속 보이게 유지
   - 회사 선택 시 `Workspace -> Upload -> Pipeline` 이동 중에도 같은 회사 문맥이 유지되게 연결
   - URL, 상태 저장, 서버 호출 파라미터가 모두 같은 회사 기준을 따르게 정리

3. `Upload` 화면을 실제 source 관리 화면으로 연결
   - 일반 업로드와 월별 업로드 진입점을 실제 API에 연결
   - source별 최근 파일명, 업로드 시각, 업로드 방식, 반영 여부를 표시
   - 파일만 올렸는지, intake까지 반영됐는지, 다음 단계가 가능한지 구분해서 설명
   - 월별 병합 대상 source는 `crm_activity`, `sales`, `target`, `prescription` 기준으로 안내 문구 고정

4. intake 결과를 사람이 읽는 설명으로 연결
   - `ready`, `ready_with_fixes`, `needs_review`, `blocked`를 화면 언어로 풀어쓴다
   - 예:
     - `ready`: 바로 실행 가능
     - `ready_with_fixes`: 자동 보정 후 실행 가능
     - `needs_review`: 검토가 필요하지만 막힌 상태는 아님
     - `blocked`: 필수 입력이 부족해 실행 불가
   - 공통 분석 구간, 후보 컬럼, 자동 보정 내역, 부족한 입력을 카드나 설명 섹션으로 노출

5. `Pipeline` 화면에서 실행 진입 흐름 연결
   - 실행 모드 선택 UI를 실제 worker runtime 지원 모드와 맞춤
   - run 시작 버튼은 실제 pipeline run 생성 API를 호출하게 연결
   - 실행 가능 여부를 boolean 한 줄로 끝내지 않고,
     왜 가능한지 / 왜 아직 불가능한지 / 무엇을 먼저 해야 하는지 안내

6. 첫 실행 경험 정리
   - run 생성 직후 사용자가 어디로 이동해야 하는지 분명히 보여줌
   - 권장 흐름:
     - 실행 요청 성공
     - 새 `run_id` 안내
     - `Run Detail`로 바로 이동하거나 계속 `Pipeline`에서 상태를 보게 함

7. 오류/예외 기본 처리
   - 업로드 실패, intake 조회 실패, pipeline 접수 실패 시
     기술 오류 문구 그대로 노출하지 말고 사용자용 설명으로 변환
   - 예:
     - "필수 파일을 아직 찾지 못했습니다"
     - "실행 요청은 접수되지 않았습니다. 잠시 후 다시 시도해 주세요"

화면별 체크포인트:

- `Workspace`
  - 회사 선택이 분명한가
  - 최근 상태와 다음 행동이 같이 보이는가
- `Upload`
  - source별 업로드 현황과 intake 판단이 이어지는가
  - 월별 업로드와 일반 업로드 차이가 설명되는가
- `Pipeline`
  - 실행 가능/불가 이유가 문장으로 보이는가
  - run 시작 이후 다음 이동 경로가 분명한가

테스트/검증:

- 회사 선택 -> 업로드 목록 조회 -> intake 결과 조회 -> pipeline run 생성까지 수동 검증
- API 실패 응답이 사용자용 문장으로 바뀌는지 확인
- `daon_pharma`, `company_000002` 두 회사 기준으로 상태 문구가 다르게 보이는지 확인

완료 기준:

- 회사 선택 -> 업로드 확인 -> 실행 진입 흐름이 실제 데이터와 연결됨
- 사용자가 현재 회사, 준비 상태, 다음 행동을 화면만 보고 이해할 수 있음

### [x] Phase 12. 실행 추적과 결과 해석 화면 연결
...
- worker가 백그라운드에서 무엇을 하고 있는지 단계별로 이해하게 만든다
- `WARN` / `FAIL`을 단순 경고가 아니라 해석 가능한 운영 정보로 보여준다
...
- `pipeline_runs`, `pipeline_run_steps` 저장 구조 고정
- validation summary와 evidence 파일 생성 완료
- `run_id` 기준 상세 조회 API 준비

세부 구현 계획:

1. `Run Detail`을 run 중심 화면으로 고정
   - 상단에서 `company_key`, `run_id`, 실행 시각, 실행 모드, 현재 전체 상태를 함께 표시
   - 사용자가 URL만 봐도 어떤 run인지 놓치지 않게 함

2. run selector / switcher 연결
   - 같은 회사 안의 최근 run 목록을 조회
   - 현재 run과 이전 run을 바꿔 보더라도 문맥이 끊기지 않게 설계
   - 최근 실행, 실패 실행, 완료 실행을 구분해서 보여줌
...
3. 단계별 진행 현황 영역 구현
   - 기본 단계:
     ...
     - payload
     - builder
   - 각 단계마다 상태, 시작/종료 시각, 요약 설명, 실패 시 이유를 표시
   - running 상태에서는 현재 어느 단계인지 강조

4. validation 결과 해석 영역 구현
   - 모듈별 `PASS`, `WARN`, `FAIL`, `APPROVED`를 그대로 쓰되 설명 문장을 같이 노출
   - 예:
     - "Territory는 필요한 재료가 부족해 전달 보류 상태입니다"
     - "Sandbox는 결과 생성 조건을 충족했습니다"
   - quality score가 있다면 숫자보다 의미를 먼저 설명

5. evidence 영역 구현
   - 근거 수치, 누락 항목, 공통 분석 구간, 전달 가능/불가 사유를 카드 또는 표로 정리
   - evidence는 "왜 이런 판정이 나왔는지"를 보여주는 근거 구역으로 명확히 분리
   - 단순 JSON 덤프 형태는 피하고, 제목 + 요약 + 세부 항목 구조로 보여줌

6. 상태 polling과 완료 전환 경험 정리
   - run이 `pending` 또는 `running`이면 일정 주기로 새로 읽음
   - 완료되면 자동으로 polling을 멈추고 결과 섹션을 펼침
   - 실패 시 "어디에서 멈췄는지"를 먼저 보여줌

7. 다음 행동 안내 추가
   - run이 완료됐으면 `Reports`, `Artifacts`로 가는 이동 버튼 제공
   - validation이 약하면 "다시 업로드 검토", "입력 보강 후 재실행" 같은 다음 행동을 안내

화면 체크포인트:

- 현재 run과 이전 run을 혼동하지 않는가
- 단계 상태와 모듈 판정이 서로 다른 의미라는 점이 분리되어 보이는가
- `WARN` / `FAIL`이 왜 나왔는지 비개발자도 읽을 수 있는가

테스트/검증:

- `pending`, `running`, `completed`, `failed` run 각각 화면 분기 확인
- `daon_pharma` 완료 run 기준 전체 결과 노출 확인
- `company_000002`처럼 검토 메시지가 필요한 회사에서 설명 문장 품질 확인

완료 기준:

- 사용자가 특정 run을 열고 실제 결과를 이해할 수 있음
- 단계 상태, 판정 이유, 다음 행동이 한 화면에서 이어짐

### [x] Phase 13. 결과물 탐색 화면 연결
...
- 사용자가 "실행 결과가 어디에 있는지" 헤매지 않게 만든다
- Builder가 만든 preview와 운영 산출물을 run 문맥으로 묶어 보여준다
...
- builder preview 파일 생성 완료
- `reports`, `artifacts`, run-context 메타 저장 구조 고정
- `Run Detail`에서 결과 화면으로 이동 가능한 상태

세부 구현 계획:

1. `Reports` 화면 연결
   - 회사 기준 최신 보고서 목록과 관련 `run_id`를 함께 표시
   - 보고서 유형별 카드 정리:
     - CRM
     - Sandbox
     - Prescription
     ...
     - RADAR
     - Total validation
   - 각 카드에 생성 시각, 연결 run, 상태, 미리보기/열기/다운로드 액션 배치
...
3. `Artifacts` 화면 연결
   - artifact를 파일 창처럼 나열하는 데서 끝내지 않고,
     파일 종류 / 생성 단계 / 관련 모듈 / 관련 run 기준으로 묶어 보여줌
   - 예:
     - validation summary
     - builder payload
     - preview html
...
4. artifact 메타 정보 정리
   - 파일명, 생성 시각, 관련 단계, 관련 모듈, 경로, 열기 가능 여부를 표시
   - 직접 열 수 없는 파일은 "메타만 확인 가능"처럼 안내

5. `Run Detail`과 결과물 화면 연결 강화
   - 특정 run에서 만든 보고서만 바로 보게 하는 필터 제공
   - 보고서 화면에서 다시 해당 run detail로 돌아갈 수 있게 연결

6. 빈 상태 / 실패 상태 처리
   - 아직 builder가 끝나지 않았으면 "보고서 생성 전" 상태로 설명
   - 파일이 없을 때는 단순 빈 화면 대신,
     아직 생성 전인지, 실패인지, 권한/경로 문제인지 구분해 설명

화면 체크포인트:

- 사용자가 run -> report -> artifact 흐름을 자연스럽게 이동할 수 있는가
- 최종 결과물과 중간 산출물이 섞여 보이지 않는가
- 보고서와 artifact 모두 `company_key`, `run_id` 문맥이 유지되는가

테스트/검증:

- `daon_pharma` 기준 preview html 및 관련 artifact 노출 확인
- 결과 없음 상태, 일부 모듈만 있는 상태, 전체 생성 완료 상태 분기 확인
- 브라우저 열기 / 다운로드 링크가 깨지지 않는지 확인

완료 기준:

- run 결과에서 실제 보고서와 산출물로 이동 가능
- 사용자가 "어떤 실행에서 나온 결과인지"를 잃지 않고 탐색할 수 있음
...
- 프론트 임시 데이터, 임시 상태명, 화면별 제각각 해석을 정리한다
- 이후 확장 단계에서 다시 데이터 구조를 흔들지 않게 기준을 고정한다
...
1. 데이터 계약 점검
   - 화면에서 쓰는 상태명과 백엔드 저장 상태명이 다르면 하나로 맞춤
   - 예:
     - `pending`, `running`, `completed`, `failed`
     - `ready`, `ready_with_fixes`, `needs_review`, `blocked`
   - 화면용 번역 문구는 따로 두되 원본 상태값은 고정

2. 공통 조회 훅 / 서버 함수 정리
   - 회사 목록, 업로드 상태, run 목록, run 상세, 보고서, artifact 조회를
     페이지마다 따로 작성하지 않고 공통 레이어로 정리
   - 캐시 키, polling 주기, 빈 상태 처리 규칙을 통일

3. mock 제거
   - 디자인용 하드코딩 데이터 제거
   - placeholder가 필요한 경우도 "아직 데이터 없음"을 보여주는 방식으로 변경

4. 경로/키 호환 정리
   - 과거 키와 현재 공식 키가 섞여 읽히는 부분이 있으면 화면 레이어에서 정리
   - `company_key`, `run_id`, `module_key`, `report_type` 사용 위치를 일관화

5. 오류 응답 표준화
   - 프론트가 API 오류를 제각각 처리하지 않도록 공통 에러 포맷과 변환 문구 정리
   - 개발용 상세 오류는 로그로 남기고,
     사용자 화면에는 이해 가능한 문장만 노출

6. 권한/환경값 준비 점검
   - 지금은 무로그인 개발 모드이지만,
     이후 인증이 붙어도 무너지지 않도록 회사/실행/결과 조회 경계를 문서화
   - Supabase 미설정, worker 미실행 같은 환경 오류도 별도 상태로 다룸

7. 데이터 검증 체크리스트 작성
   - 화면별로 어떤 API와 어떤 테이블/파일이 연결되는지 짝을 맞춰 기록
   - 이후 버그가 났을 때 어디를 봐야 하는지 찾기 쉽게 정리

완료 기준:

- 주요 페이지가 실제 데이터로 동작
- mock 없이도 `Workspace`, `Upload`, `Pipeline`, `Run Detail`, `Reports`, `Artifacts`가 이어짐
- 상태값과 설명 문구 기준이 화면 전반에서 통일됨
...
- 사용자가 단순 결과 확인을 넘어서 "무엇을 먼저 봐야 하는지" 판단할 수 있게 한다
- RADAR를 별도 계산 엔진이 아니라, 이미 검증된 결과를 해석하는 우선순위 레이어로 붙인다

선행 조건:

- `Phase 6 ~ Phase 9` 공식 결과 구조 고정
- validation summary와 result asset가 안정적으로 생성됨
- RADAR 입력이 KPI 원천을 다시 계산하지 않도록 경계 고정

세부 구현 계획:

1. RADAR 역할 재확인
   - RADAR는 새 KPI 계산기가 아님
   - validation 이후 결과를 읽고
     "어디에 신호가 있는지", "무엇이 먼저 처리 대상인지", "어떤 선택지가 있는지"
     정리하는 해석 레이어로 구현

2. signal detection 구조 구현
   - 모듈별 결과에서 이상 징후, 기회 신호, 주의 구간을 추출
   - 신호 유형 예시:
     - 목표 대비 급격한 하락
     - 특정 권역 편중
     - prescription 흐름 이상
     - 실행은 완료됐지만 해석 우선순위가 높은 항목

3. issue prioritization 규칙 구현
   - 모든 이슈를 같은 무게로 보여주지 않고
     영향도, 긴급도, 데이터 신뢰도 기준으로 우선순위를 매김
   - 우선순위는 숫자만이 아니라
     "지금 먼저 볼 것", "추가 확인 필요", "추후 검토"처럼 운영 언어로 보여줌

4. decision option 템플릿화
   - 신호마다 바로 실행 가능한 선택지를 붙임
   - 예:
     - 원본 입력 재검토
     - 특정 권역 상세 확인
     - prescription 흐름 점검
     - 다음 run에서 추가 데이터 보강
   - 선택지는 조언 문장이 아니라, 화면에서 클릭 가능한 후속 행동 힌트로 연결 가능하게 설계

5. RADAR 결과 노출 방식 정리
   - `Reports` 또는 전용 `RADAR` 영역에서
     핵심 신호 요약, 우선순위 목록, 판단 옵션을 보여줌
   - run 기준 결과와 회사 기준 누적 해석을 구분해 보여줄지 결정

6. 근거 연결
   - RADAR 문장은 항상 validation/evidence/artifact/report 중 최소 하나와 연결
   - 근거 없는 요약 문장은 허용하지 않음

테스트/검증:

- `daon_pharma` 기준 RADAR 신호 생성 확인
- 신호가 없을 때도 억지 인사이트를 만들지 않는지 확인
- 동일 run 재조회 시 우선순위와 근거 연결이 안정적으로 보이는지 확인

완료 기준:

- 추가 인사이트를 생성할 수 있음
- 신호, 우선순위, 다음 선택지가 근거와 함께 제시됨
...
- 제품이 "기능은 되지만 거칠게 연결된 상태"에서 "운영 가능한 제품 형태"로 올라가게 만든다
...
- `Phase 11 ~ Phase 14` 주요 사용자 흐름 완료
- 공통 레이아웃과 데이터 연결 방식 고정

세부 구현 계획:

1. 예시 HTML 기반 스타일 정교화
   - 이미 구현한 화면을 디자인 예시의 분위기에 더 가깝게 다듬음
   - 단, HTML 복붙이 아니라 공통 컴포넌트와 섹션 단위 개선으로 진행
   - 우선 보강 대상:
     - 상태 배지와 설명 박스
     - run 단계 타임라인
     - 보고서/산출물 카드
...
2. 관리용 `Admin` 화면 도입
   - 운영자가 회사, run, artifact 상태를 한 번에 점검할 수 있는 관리 화면 추가
   - 1차 범위:
     - 회사별 최근 run 요약
     - 실패 run 확인
     - worker 처리 상태 확인
...
3. Artifacts/Admin 확장
   - artifact를 일반 사용자용 보기와 운영 관리용 보기로 나눌지 판단
   - 운영 관리용 보기에서는 파일 수, 누락 파일, 최근 생성 실패 여부 등 점검 기능 강화

4. 운영 편의 기능 추가
   - run 재조회, 빠른 이동, 최근 실패 필터, 최근 생성 보고서 필터 등
   - 반복 운영 작업을 줄이는 쪽으로 기능을 보강

5. 화면 언어 정리
   - 페이지마다 비슷한 뜻을 다른 말로 쓰지 않게 정리
   - 예:
     - 실행 / run / 파이프라인
     - 산출물 / artifact
     - 보고서 / preview / 최종 결과
...
- Agent를 일반 채팅창이 아니라 `운영 해석 도구`로 구현한다

선행 조건:

- `Run Detail`, `Reports`, `Artifacts`, `RADAR` 연결 완료
- `run_id` 기준 근거 자산 조회가 가능함
- Agent가 읽을 수 있는 context 묶음 구조 준비

핵심 원칙:

- Agent는 독립 채팅 기능이 아니다
- 답변은 반드시 현재 run, validation 결과, artifact, report 문맥을 기반으로 해야 한다
- 근거 없는 일반론 답변은 허용하지 않는다
...
1. Agent 입력 문맥 고정
   - 최소 입력:
     - 현재 `company_key`
     - 현재 `run_id`
     - 연결된 validation summary
     - 주요 artifacts
     - report context
   - 사용자가 다른 run으로 이동하면 Agent 문맥도 함께 바뀌게 설계

2. 질문/응답 UX 설계
   - 사용자가 자유 질문은 할 수 있지만,
     시작점은 "이 run에서 무엇이 문제인가", "왜 WARN인가", "무엇을 먼저 봐야 하나" 같은 운영형 질문을 쉽게 던질 수 있게 함
   - 추천 질문 버튼 또는 빠른 질문 템플릿 제공 검토

3. 근거 연결 응답 구현
   - 답변 안에 어떤 artifact / validation 항목 / report를 근거로 썼는지 명시
   - 가능하면 해당 근거 화면으로 바로 이동하는 링크 제공

4. 응답 범위 제한
   - 현재 run과 무관한 일반 분석으로 새어 나가지 않게 제한
   - 데이터에 없는 내용은 추정으로 단정하지 않도록 설계
   - 필요한 경우 "현재 run 기준 근거가 부족하다"는 답을 하게 함
...
5. 운영 해석 보조 UX 구현
   - 긴 문장만 뿌리지 말고,
     ...
     다음 행동
     형식으로 응답을 구조화
   - 비개발자 기준으로 이해 가능한 말투 유지

6. 기록/재조회 구조
   - Agent 대화 로그를 `run_id` 기준으로 저장할지 결정
   - 이후 같은 run을 다시 열었을 때 이전 질문 이력을 보여줄지 범위 결정

테스트/검증:

- 같은 질문을 다른 run에서 했을 때 답변 문맥이 바뀌는지 확인
- 근거가 없는 질문에 대해 무리한 답을 하지 않는지 확인
- artifact / report 링크가 실제로 이어지는지 확인

완료 기준:

- 사용자가 특정 run 기준으로 질문하고 근거와 함께 해석을 받을 수 있음
- Agent가 run 문맥 없는 일반 채팅창처럼 보이지 않음
...
- 개별 기능 완료를 넘어, 장애 대응과 유지보수까지 가능한 상태로 만든다
...
1. 에러 처리 전면 정리
   - 업로드 실패, run 실패, report 미생성, artifact 누락, Supabase 미연결, worker 미동작 등
     대표 장애 케이스를 화면/서버/worker 기준으로 정리
   - 사용자용 오류 문구와 개발자용 로그를 분리
...
3. 테스트 체계 보강
   ...
   - 서버:
     - API 응답 구조 테스트
   - worker:
     - run 상태 전환 / 실패 처리 / 중복 실행 방지 테스트
   - 통합:
     - 업로드 -> run -> 결과 조회 최소 흐름 검증

4. 로그 / 모니터링 정리
   - run 생성, 단계 전환, 실패 지점, builder 결과 생성 여부 로그 정리
   - 운영자가 나중에 "어디서 깨졌는지" 찾을 수 있게 로그 키를 통일
...
6. 인증/권한 확장 준비
   - 지금은 무로그인이지만,
     이후 회사별 접근 제한, 운영자 권한, 결과 다운로드 권한이 붙을 수 있게 구조 준비
   - 현재 단계에서는 실제 인증 도입보다 확장 포인트 정리가 우선
...
7. 출시 전 점검 체크리스트 작성
   ...
   - report/artifact 열람
   - Agent 해석
   전 구간을 한 번에 점검하는 운영 체크리스트 작성

완료 기준:

- 운영 중 깨지기 쉬운 구간이 줄어들고 유지보수 가능한 상태가 됨
- 장애 시 어디를 확인해야 하는지 문서와 로그 기준이 마련됨
